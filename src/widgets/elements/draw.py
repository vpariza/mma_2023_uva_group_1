...
class Draw(JSCSSMixin, MacroElement):
    """
    This is an updated implementation of the Original Draw Component.
    This Element's class avoids alerting on the web application which eventually
    avoids using a dialog box when clicking on an element on the map. The goal
    is to silently use the information logged but not alerting that information
    as a dialog box.
